...
import type { SidebarProps, SidebarSection } from "./Sidebar.types";
...
export function Sidebar({
  ...
  return (
    <>
      {isMobile && isOpen && <div className="fixed inset-0 bg-black/50 z-40" onClick={onToggle} />}
      ...
      <aside className={clsx("sidebar", isOpen && "sidebar--open")}>
        ...
        <nav className="sidebar__nav">
          {navSections.map((section) => (
            <div key={section.title} className="mb-4">
              <div className="sidebar__section-title" onClick={() => toggleSection(section.title)}>
                {isOpen ? (
                  <>
                ...
                <div className="mt-1">
                  {isOpen &&
                    section.items.map((item) => {
                      ...
                      return (
                        <a key={item.id} href={item.path} className="sidebar__link">
                          <Icon size={16} />
        ...
        <div className="sidebar__footer">
          ...
          <button onClick={onToggleTheme} className={clsx("sidebar__item", !isOpen && "justify-center")}>
