...
export function StrengthBar({ password = '' }: StrengthBarProps): JSX.Element | null {
      ...
      <div className="auth-form__strength-bars">
        {[1, 2, 3].map((i) => (
          <div 
            ...
            style={{ backgroundColor: i <= level ? STRENGTH_COLORS[level] : 'var(--border-strong)' }} 
          />
        ))}
      </div>
      <span className="auth-form__strength-label" style={{ color: STRENGTH_COLORS[level] }}>
