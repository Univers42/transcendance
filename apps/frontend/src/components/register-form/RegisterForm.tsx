...
export function RegisterForm({ onSwitch }: RegisterFormProps): JSX.Element {
  const [form, setForm] = useState<RegisterFormState>({ name: '', email: '', password: '', confirm: '', terms: false });
  ...
  function set<K extends keyof RegisterFormState>(k: K, v: RegisterFormState[K]) {
    setForm(f => ({ ...f, [k]: v }));
